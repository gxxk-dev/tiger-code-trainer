...
export function CompletionView({ request, result, onClose, onContinue, continueLabel, className }: CompletionViewProps) {
  ...
  const nextStep = request.origin === 'daily'
    ...
    : request.stageId === 'formula'
      ? '下一步：每次认识 5 个常用字根。'
      : ''
  const actionIcon = continueLabel === '完成今日训练' ? Check : ArrowRight
  ...
  return (
    <main className={clsx('mx-auto grid min-h-[calc(100dvh-4rem)] max-w-3xl content-center gap-8 px-4 py-10 text-center sm:px-6 lg:px-8', className)}>
      <div className="grid justify-items-center gap-2">
        <AppIcon icon={Check} inline={false} className="stroke-emerald-600 dark:stroke-emerald-300" />
        <h1 ref={headingRef} tabIndex={-1} className="text-3xl font-semibold text-zinc-950 outline-none dark:text-white">本轮完成</h1>
        <p className="text-base text-zinc-600 sm:text-sm dark:text-zinc-400">{request.title}</p>
        {nextStep ? <p className="text-base font-medium text-brand-700 sm:text-sm dark:text-brand-300">{nextStep}</p> : null}
      </div>
      ...
      <div className="flex justify-center">
        <Button
          variant="primary"
          leadingIcon={onContinue ? undefined : ArrowLeft}
          trailingIcon={onContinue ? actionIcon : undefined}
...
function ResultStat({ label, value }: { label: string; value: string }) {
  return <div className="min-w-0 px-3"><dt className="truncate text-base text-zinc-500 sm:text-sm dark:text-zinc-400">{label}</dt><dd className="mt-1 truncate text-2xl font-semibold text-zinc-950 tabular-nums dark:text-white">{value}</dd></div>
}
