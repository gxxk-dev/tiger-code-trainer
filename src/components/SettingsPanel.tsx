        ...
        <div className="grid gap-8 p-5">
          ...
          <section aria-labelledby="plan-title" className="grid gap-5 border-t border-zinc-950/8 pt-7 dark:border-white/8">
            <div>
              ...
              <p className="mt-1 text-base text-pretty text-zinc-600 sm:text-sm dark:text-zinc-400">强度同时决定每日题量和新内容上限，到期复习始终优先。</p>
            </div>
            <TrainingIntensityControl
              value={progress.settings.dailyMinutes}
              onChange={(minutes) => onUpdate(settingsForIntensity(minutes))}
            />
            <SettingToggle
              id="auto-advance"
              label="答对后自动下一题"
