...
const lookupTabs = [
  { value: 'characters', label: '汉字' },
  { value: 'roots', label: '字根' },
] as const
...
export function LookupView() {
  ...
  const tabRefs = useRef<Record<LookupMode, HTMLButtonElement | null>>({ characters: null, roots: null })
  ...
  const handleTabKeyDown = (event: ReactKeyboardEvent<HTMLButtonElement>, currentMode: LookupMode) => {
    const currentIndex = lookupTabs.findIndex((tab) => tab.value === currentMode)
    const nextIndex = event.key === 'ArrowRight'
      ? (currentIndex + 1) % lookupTabs.length
      : event.key === 'ArrowLeft'
        ? (currentIndex - 1 + lookupTabs.length) % lookupTabs.length
        : event.key === 'Home'
          ? 0
          : event.key === 'End'
            ? lookupTabs.length - 1
    ...
    const nextMode = lookupTabs[nextIndex].value
    selectMode(nextMode)
    ...
    <div className="mx-auto grid max-w-6xl gap-8 px-4 py-8 sm:px-6 sm:py-10 lg:px-8 lg:py-12">
      ...
      <div className="flex flex-col gap-3 sm:flex-row sm:items-center">
        <div className="flex shrink-0 gap-1 rounded-lg bg-zinc-950/5 p-1 dark:bg-white/6" role="tablist" aria-label="查询类型">
          {lookupTabs.map(({ value, label }) => (
            <button
              ...
              aria-controls={`lookup-panel-${value}`}
        ...
        <label htmlFor="lookup-input" className="relative min-w-0 flex-1">
          ...
          <AppIcon icon={Search} inline={false} className="pointer-events-none absolute top-1/2 left-3 -translate-y-1/2 stroke-zinc-500" />
          <input
            id="lookup-input"
            name="lookup"
            type="search"
      ...
      <p className="sr-only" aria-live="polite" aria-atomic="true">
        {loading ? '正在载入查码数据。' : `找到 ${mode === 'characters' ? characterResults.length : rootResults.length} 项。`}
      </p>

      <section id="lookup-panel-characters" role="tabpanel" aria-labelledby="lookup-tab-characters" aria-busy={loading} tabIndex={0} hidden={mode !== 'characters'}>
              ...
              {characterResults.map((entry) => (
                <li key={entry.char} className="rounded-lg bg-white p-5 ring-1 ring-zinc-950/8 dark:bg-white/4 dark:ring-white/8">
                  <div className="flex min-w-0 items-start gap-4">
                    <p className="flex size-14 shrink-0 items-center justify-center font-root text-4xl font-medium text-zinc-950 dark:text-white">{entry.char}</p>
                      ...
                      <p className="mt-2 font-root text-base text-zinc-800 dark:text-zinc-200">
                        {Array.from(entry.decomposition).map((root, index) => (
                          <span key={`${root}-${index}`}>
                            ...
                            {displayRootGlyph(root, entry.rootCodes[index])}<span className="font-mono text-zinc-500 dark:text-zinc-400"> {entry.rootCodes[index]?.toLowerCase()}</span>
                          </span>
                        ))}
                      </p>
                      <p className="mt-2 text-base text-zinc-500 sm:text-sm dark:text-zinc-400">{entry.pinyin}，全部编码 {entry.codes.join(' / ')}</p>
                    </div>
                  </div>
                </li>
              ))}
      ...
      <section id="lookup-panel-roots" role="tabpanel" aria-labelledby="lookup-tab-roots" tabIndex={0} hidden={mode !== 'roots'}>
          <div className="-mx-4 overflow-x-auto px-4 sm:-mx-6 sm:px-6 lg:-mx-8 lg:px-8">
            <div className="flex min-w-max gap-1 pb-2" aria-label="按大码筛选">
              <button type="button" aria-pressed={!keyFilter} onClick={() => setKeyFilter('')} className={keyButtonClass(!keyFilter)}>全部</button>
              {Array.from('abcdefghijklmnopqrstuvwxyz').map((key) => (
                <button type="button" key={key} aria-pressed={keyFilter === key} onClick={() => setKeyFilter(key)} className={keyButtonClass(keyFilter === key)}>{key.toUpperCase()}</button>
              ))}
            </div>
          </div>
          <ul role="list" className="mt-4 grid gap-2 sm:grid-cols-2 xl:grid-cols-3">
            {rootResults.map((root, index) => (
              <li key={`${root.root}-${root.code}-${index}`} className="flex min-w-0 items-center gap-4 rounded-md bg-white p-3 ring-1 ring-zinc-950/8 dark:bg-white/4 dark:ring-white/8">
                <p className="flex size-11 shrink-0 items-center justify-center font-root text-2xl font-medium text-zinc-950 dark:text-white">{displayRootGlyph(root.root, root.code)}</p>
                <div className="min-w-0 flex-1">
                  <p className="truncate font-mono text-base font-semibold text-brand-700 dark:text-brand-300">{root.code}</p>
                  <p className="mt-1 truncate font-root text-base text-zinc-500 sm:text-sm dark:text-zinc-400">{root.examples.join('、') || root.variants || '字根'}</p>
                </div>
              </li>
            ))}
          </ul>
      ...
      <section aria-labelledby="chart-title" className="border-t border-zinc-950/8 pt-8 dark:border-white/8">
        ...
        <a href={lightRootChartUrl} target="_blank" rel="noreferrer" aria-label="在新标签页打开浅色官方字根图" className="mt-5 block overflow-hidden rounded-[min(1vw,var(--radius-lg))] bg-white outline-1 -outline-offset-1 outline-zinc-950/10 focus-visible:outline-2 focus-visible:outline-offset-2 focus-visible:outline-brand-500 dark:hidden">
          <img src={lightRootChartUrl} width="2600" height="938" loading="lazy" alt="" className="w-full" />
        </a>
        <a href={darkRootChartUrl} target="_blank" rel="noreferrer" aria-label="在新标签页打开深色官方字根图" className="mt-5 block overflow-hidden rounded-[min(1vw,var(--radius-lg))] bg-zinc-950 outline-1 -outline-offset-1 outline-white/10 focus-visible:outline-2 focus-visible:outline-offset-2 focus-visible:outline-brand-500 not-dark:hidden">
          <img src={darkRootChartUrl} width="2600" height="938" loading="lazy" alt="" className="w-full" />
        </a>
        <a href="https://www.tiger-code.com/" target="_blank" rel="noreferrer" className="mt-2 flex w-fit items-center gap-1 rounded-sm text-zinc-500 outline-none hover:text-zinc-950 focus-visible:outline-2 focus-visible:outline-offset-2 focus-visible:outline-brand-500 dark:text-zinc-400 dark:hover:text-white">
          <p className="text-base sm:text-sm">来源：虎码官网</p>
...
function keyButtonClass(active: boolean): string {
  return clsx(
    'min-h-12 min-w-12 rounded-md px-2 font-mono text-base font-medium outline-none ring-1 ring-zinc-950/8 focus-visible:outline-2 focus-visible:outline-brand-500 pointer-fine:min-h-8 pointer-fine:min-w-8 sm:text-sm dark:ring-white/8',
