  ...
  return (
    <div className="mx-auto grid max-w-6xl gap-10 px-4 py-8 sm:px-6 sm:py-10 lg:px-8 lg:py-12">
      <header className="flex flex-col gap-5 border-b border-zinc-950/8 pb-8 sm:flex-row sm:items-end sm:justify-between dark:border-white/8">
        ...
        <div className="flex gap-1 overflow-x-auto rounded-lg bg-zinc-950/5 p-1 dark:bg-white/6" aria-label="统计时间范围">
        ...
        <>
          ...
          <section aria-labelledby="activity-title" className="grid gap-5 border-t border-zinc-950/8 pt-8 dark:border-white/8">
            <div>
              ...
              <p className="mt-1 text-base text-zinc-600 sm:text-sm dark:text-zinc-400">柱高表示当天完成题数。</p>
            </div>
            <ActivityBars sessions={progress.sessions} />
          </section>

          <section aria-labelledby="history-title" className="border-t border-zinc-950/8 pt-8 dark:border-white/8">
...
function ActivityBars({ sessions }: { sessions: SessionRecord[] }) {
  ...
  return (
    <div className="grid h-44 grid-cols-7 items-end gap-2 border-b border-zinc-950/10 dark:border-white/10">
      {days.map((day) => (
        <div key={day.date.toISOString()} className="flex h-full min-w-0 flex-col justify-end gap-2">
          <div className="flex min-h-0 flex-1 items-end">
            <div
              className="w-full rounded-t-sm bg-blue-500/80"
              style={{ '--bar-height': `${Math.max(day.count ? 8 : 2, day.count / max * 100)}%`, height: 'var(--bar-height)' } as React.CSSProperties}
              title={`${day.count} 题`}
            />
          </div>
