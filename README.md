...
- 记忆 Hint：用 26 个键位锚点、官网拼音音托和同码伙伴串起 241 个字根
- 逐步拆字：展开整字、字根双码、取码公式与最终全码的完整过程
- 练码模式：切到英文后直接输入虎码编码
...
1. 看一遍横、竖、撇、点/捺、折及对应编码和学习联想
2. 答案保持显示时，照着输入一次；这一段不计分
3. 最后遮住答案独立回忆，再进入取码公式

之后每批新内容都会先讲解和跟打，只有已经见过的内容与到期复习会直接进入测试。首页给出当前下一步；完整课程仍可自由跳转。
