...
- 肌肉记忆：每根两次可见跟打、两轮交错盲打，错后立即修正并稍后重测
- 例字定位：把官网例字拆开显示，明确标出当前字根在整字中的位置
- 逐步拆字：展开整字、字根双码、取码公式与最终全码的完整过程
- 切字入门：对照普通拆分和需要剪断连续笔画的切字
- 练码模式：切到英文后直接输入虎码编码
...
1. 看一遍横、竖、撇、点/捺、折的字形、编码和例字
2. 答案保持显示时，每组连续输入两次；这一段不计分
3. 打乱顺序完成两轮盲打；答错时先照着正确编码重敲，稍后再测
4. 完成后进入取码公式，再通过普通拆分与切字对照学习拆字
