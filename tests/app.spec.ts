...
test('split learner can request the worked process without gaining mastery', async ({ page }) => {
  ...
  const retrySplitChoices = page.getByRole('group', { name: '为“秃”按顺序选择字根' })
  await retrySplitChoices.getByRole('button', { name: /禾/ }).click()
  await retrySplitChoices.getByRole('button', { name: /几/ }).click()
  await page.getByRole('button', { name: '提交拆分' }).click()
  await page.getByRole('button', { name: '下一题' }).click()
...
test('incorrect split feedback has no serious accessibility violations', async ({ page }) => {
  ...
  const choices = page.getByRole('group', { name: '为“秃”按顺序选择字根' })
  await choices.getByRole('button', { name: /几/ }).click()
  await choices.getByRole('button', { name: /禾/ }).click()
  await page.getByRole('button', { name: '提交拆分' }).click()
...
test('course exposes formula and split practice', async ({ page }) => {
  ...
  await page.getByRole('button', { name: '看完过程，开始选字根' }).click()
  const splitChoices = page.getByRole('group', { name: '为“秃”按顺序选择字根' })
  await expect(splitChoices).toBeVisible()
  await expect(page.getByText('1 / 1', { exact: true })).toBeVisible()
  await expect(page.getByRole('textbox')).toHaveCount(0)
  await expect(splitChoices.getByRole('button', { name: '禾 + 几' })).toHaveCount(0)
  await splitChoices.getByRole('button', { name: /禾/ }).click()
  await splitChoices.getByRole('button', { name: /几/ }).click()
  await expect(page.getByText('当前拆分').locator('..')).toContainText('禾 + 几')
  await page.getByRole('button', { name: '提交拆分' }).click()
